...
const FindNutritionistPage = ({ navigation }) => {
  ...
  const validateTime = (time) => {
    const timeRegex = /^(0?[1-9]|1[0-2]):[0-5][0-9] ?(AM|PM|am|pm)$/i; // HH:MM AM/PM format
  ...
  const handleSchedule = async () => {
    ...
    if (!validateDate(scheduleDetails.date)) {
      alert('Invalid date format! Use YYYY-MM-DD.');
      return;
    }
    if (!validateTime(scheduleDetails.time)) {
    ...
    try {
      const response = await fetch('http://10.54.23.76:3000/api/schedule', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        alert(`Appointment Scheduled with ${selectedNutritionist.name} on ${scheduleDetails.date} at ${scheduleDetails.time}`);
      } else {
